...
public class HauntedSlashParticle<T extends SlashParticleEffect> extends SpriteBillboardParticle {
    private final Vec3d direction;
    private float yaw;
    private SpriteProvider provider;
    private float pitch;
    protected HauntedSlashParticle(ClientWorld clientWorld, double d, double e, double f, SpriteProvider spriteProvider, double xd, double yd, double zd, SlashParticleEffect effect,SpriteProvider provider) {
        ...
        dir = dir.normalize();
        this.velocityX = dir.x*0.15;
        this.velocityY = dir.y*0.15;
        this.velocityZ = dir.z*0.15;
        ...
        this.yaw = (float)Math.atan2(dir.x, dir.z)+3.14f/2f;
    }
    ...
    public void buildGeometryAlt(VertexConsumer vertexConsumer, Camera camera, float tickDelta,float zrot) {
        ...
        quaternionf = new Quaternionf().rotateYXZ(this.yaw,zrot,this.pitch);
    ...
    public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
        buildGeometryAlt(vertexConsumer,camera,tickDelta,-.3f);
        buildGeometryAlt(vertexConsumer,camera,tickDelta,.3f);
    }
    ...
    public static class Factory implements ParticleFactory<SlashParticleEffect> {
        ...
        public Particle createParticle(SlashParticleEffect particleType, ClientWorld level, double x, double y, double z,
                                       double dx, double dy, double dz) {
            return new HauntedSlashParticle<>(level, x, y, z, this.sprites, dx, dy, dz,particleType,sprites);
        }
    }
}
